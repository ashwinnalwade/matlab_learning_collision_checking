...
env_dataset = strcat(getenv('collision_checking_dataset_folder'), '/heur_dataset_2gap/');

%% Wall with gaps
width = 0.2;
gap_height = 0.1; %half the height
...
for i = 1:num_worlds
    ...
    % Sample a config
    if (rand() <= 0.7)
        shape_array = [shape_array get_wall_with_shifting_gaps( bbox, 0.5, width, gap_height, [0.1 0.3] )];
    else
        shape_array = [shape_array get_wall_with_shifting_gaps( bbox, 0.5, width, gap_height, [0.7 0.9] )];
    end
